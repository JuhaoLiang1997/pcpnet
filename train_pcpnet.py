...
def parse_arguments():
    ...
    parser.add_argument('--name', type=str, default='single_scale_normal', help='training run name')
...
def train_pcpnet(opt, wb_logger):
    ...
    for epoch in range(opt.nepoch):
        ...
        for train_batchind, data in train_enum:
            ...
            wb_logger.log({"epoch": epoch, "train_batchind": train_batchind, "train loss": loss.item(), "learning_rate": get_lr(optimizer)})
            ...
            while test_fraction_done <= train_fraction_done and test_batchind+1 < test_num_batch:
                ...
                wb_logger.log({"epoch": epoch, "test_batchind": test_batchind, "test loss": loss.item()})
...
if __name__ == '__main__':
    ...
    wb_logger = wandb.init(project='PCPNet')
    wb_logger.config.update(train_opt)
    train_pcpnet(train_opt, wb_logger)
